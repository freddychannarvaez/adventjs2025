...
function timeUntilTakeOff(
  ...
  function elfDateTimeToDate(edt: ElfDateTime): Date {
    const start = edt
      .replace(asteriscRe, "-")
      .replace(atRe, "T")
      .replace(barRe, ":")
  ...
  const start = elfDateTimeToDate(fromTime).valueOf();
  const end = elfDateTimeToDate(takeOffTime).valueOf();
  const result = Math.floor((end - start) / 1000);
  return result;
}
